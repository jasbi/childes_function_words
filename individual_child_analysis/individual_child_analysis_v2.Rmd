...
Possible that one of the top children found here may produce more tokens outside of the 1-4 age range
```{r}
...
### Plot Version 2
```{r cumulative_ppt_trajectories, warning=FALSE, message=FALSE, fig.width=14, fig.height=14}
cumulative_ppt <- ggplot(child_relfreq_filtered, aes(x = target_child_age, y = cumulative_ppt, color = target_child_name)) +
  geom_line() +
  scale_color_grey(start = 0.2, end = 0.8) +
  facet_wrap(~ word, ncol = 2, scales = "free_y") +
  theme_bw(base_size = 22) +  # set base font size to be twice the usual default (default = 11)
  labs(#title = "Cumulative Relative Frequency of Selected Function Words\nTop 10 Children",
       x = "Age (months)",
       y = "Cumulative PPT",
       color = "Child") +
  theme(plot.title = element_text(hjust = 0.5, size = 24),
        ...
        axis.text = element_text(size = 18), # ticks on axis
        legend.title = element_text(size = 22),
        legend.text = element_text(size = 18),
        strip.text = element_text(size = 20), # panels
        legend.position = "right")
print(cumulative_ppt)
```


## Aggregate Cumulative PPT Trajectories Across Selected Children (Per Function Word, Per Age)

Computes for each function word and child age, the combined cumulative PPT aggregate across the top children, starting from the already grouped/filtered data of selected children.
```{r}
...
plots <- map(function_words, function(current_word) {
  p <- child_relfreq %>%
...
plots_aggregate_overlay <- map(function_words, function(current_word) {
  p <- child_relfreq %>%
    filter(word == current_word, target_child_name %in% top_ten_children$target_child_name) %>%
    ggplot(aes(x = target_child_age, y = cumulative_ppt, color = target_child_name)) +
    geom_point() +
...
### Individual Level Trend: "and"
```{r individual_cumulative_ppt_pooled_aggregate_overlay_and_only, fig.width=14, fig.height=14, eval=TRUE, warning=FALSE, message=FALSE}
and <- child_relfreq %>%
  filter(word == "and", target_child_name %in% top_ten_children$target_child_name) %>%
  ggplot(aes(x = target_child_age, y = cumulative_ppt, color = target_child_name)) +
  geom_line() +
  geom_line(data = aggregate_from_tokens %>% filter(word == "and"),
            aes(x = target_child_age, y = cumulative_ppt, group = NULL),
            inherit.aes = FALSE,
            color = "black",
            size = 1.2) +
  scale_color_grey(start = 0.2, end = 0.8) +
  labs(title = "Individual Level Trend for 'and'",
       x = "Age (months)",
       y = "Cumulative PPT",
       color = "Child") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5, size = 32),
        ...
        axis.text = element_text(size = 26), # ticks on axis
...
plots_pooled_overlay <- map(function_words, function(current_word) {
  p <- child_relfreq %>%
    filter(word == current_word, target_child_name %in% top_ten_children$target_child_name) %>%
    ggplot(aes(x = target_child_age, y = cumulative_ppt, color = target_child_name)) +
    geom_point() +
