...
function_words_df_collapsed <- function_words_df %>%
  distinct(gloss, word_category) %>%
  group_by(gloss) %>%
  summarise(word_category = paste(word_category, collapse = ", "), .groups = 'drop')

# all words just have one type, join by gloss
age_df <- 
  age_df %>% 
  left_join(function_words_df_collapsed, by = "gloss")
```
