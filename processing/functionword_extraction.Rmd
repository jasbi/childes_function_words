...
```{r}
# Change cannot to can not (separate by a space)
...
contractions <-
  english_tokens %>%
  filter(str_detect(gloss, "n't|'ll|'s|'d|'m|'re"))
...
contracted$gloss <- str_remove_all(contracted$gloss, "n't|'ll|'s|'d|'m|'re")

# in the other remove the contracted

contractions$gloss <- str_extract(contractions$gloss, "n't|'ll|'s|'d|'m|'re")

# put the dataframes back together

contracted_contractions <-
  rbind(codntracted, contractions)

# bind them with the original that was without contractions

english_tokens <-
  rbind(english_tokens, contracted_contractions)
```
